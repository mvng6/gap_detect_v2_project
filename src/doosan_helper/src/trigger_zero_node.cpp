...
{
    // 노드 초기화. 이름: "trigger_zero_node"
    ros::init(argc, argv, "trigger_zero_node");
    ros::NodeHandle nh;
    ...
    // 3. 메시지 발행 ("0이라고 외치기")
    ROS_INFO("Publishing command '0' to /katech/robot_command");
    command_pub.publish(msg);

    // 발행 후 바로 종료되지 않도록 잠시 대기 (선택사항)
    ros::Duration(0.5).sleep(); 

    ROS_INFO("Command '0' published. Exiting.");
