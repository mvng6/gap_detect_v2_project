...
본 시스템은 **ROS Topic 통신**을 사용하여 중앙 관제 노드와 각 로봇 간의 명령/상태를 주고받습니다.

#### 📻 Topic 통신이란?

**라디오 방송**처럼 작동하는 일방향 통신 방식입니다:

```
[발행자] ──────► 📻 Topic Channel ──────► [구독자들]
          (메시지 발행)              (메시지 수신)
```
...
명령 발행   상태 구독
    ↓         ↑
┌─────────────────────┐
│  /mobile/cmd        │ → 모바일 로봇에게 명령
│  /katech/robot_cmd  │ → 두산 로봇에게 명령
│  /mobile/status     │ ← 모바일 로봇 상태 수신
│  /doosan/status     │ ← 두산 로봇 상태 수신
└─────────────────────┘
```
...
| 특징 | 설명 |
|------|------|
| 🚀 **빠른 응답** | 답장을 기다리지 않아 즉각 다음 작업 가능 |
| 🔄 **비동기 처리** | 로봇이 작업 중에도 상태를 계속 확인 가능 |
| 📡 **멀티캐스트** | 하나의 명령을 여러 로봇에게 동시 전달 가능 |
| 🎛️ **느슨한 결합** | 로봇이 연결 해제되어도 시스템 전체가 멈추지 않음 |

#### 🔄 실제 동작 흐름

```
1️⃣ 중앙 관제: "모바일아, 0.3m 전진!" 
   → /mobile/cmd 발행

2️⃣ 모바일 로봇: 명령 수신 → 이동 시작
   → /mobile/status: "MOVING" 발행

3️⃣ 중앙 관제: 상태 확인 → 완료 대기
   (이 시간에 다른 작업 가능!)

4️⃣ 모바일 로봇: 이동 완료
   → /mobile/status: "COMPLETED" 발행

5️⃣ 중앙 관제: 완료 확인 → 다음 로봇 제어
```

> 💡 **왜 Service/Action이 아닌 Topic을?**  
> Service는 응답을 기다리는 동안 블로킹되고, Action은 구현이 복잡합니다.  
> 우리 시스템은 **간단하고 빠른 순차 제어**가 목적이므로 Topic이 최적입니다.
